...
// Your answers from earlier runs. Gitignored, so it is yours alone and updates never touch it.
const SAVED_ANSWERS = path.join(repoRoot, 'answers.env');
...
Options:
  ...
  --answers FILE          Read answers from a KEY=value file (see answers.example.env).
                          Without it, answers.env is read when present and a real run
                          saves your answers back to it, so a re-run (for example after
                          ./update.sh) asks only questions you have not answered yet.
  --yes, --non-interactive
                          Never prompt: use answers, then defaults. Secrets come from env vars only.
  --save-answers FILE     Also write the answers (never secrets) to FILE for reuse.
...
// Earlier answers are kept, even for modules or conditions not in play this run; this
// run's answers replace them; keys no question asks any more (and secrets) are dropped.
function saveAnswers(file, previous, values, keep) {
  const merged = new Map();
  for (const [k, v] of previous) if (keep(k)) merged.set(k, v);
  for (const [k, v] of Object.entries(values)) {
    if (!keep(k)) continue;
    merged.set(k, Array.isArray(v) ? v.join(',') : typeof v === 'boolean' ? (v ? 'yes' : 'no') : v);
  }
...
async function main() {
  ...
  const answersFile = opts.answers || (fs.existsSync(SAVED_ANSWERS) ? SAVED_ANSWERS : null);
  const usingSaved = !opts.answers || path.resolve(opts.answers) === SAVED_ANSWERS;
  const loaded = answersFile ? parseAnswersFile(answersFile) : new Map();
  const answers = new Map(loaded);
  ...
  if (answersFile && usingSaved) console.log(`  using your saved answers from ${path.basename(answersFile)} (${loaded.size}); only new questions are asked`);
  const selected = await selectModules(ctx, modules);
  if (usingSaved && loaded.has('MODULES') && !opts.modules) {
    const others = modules.filter((m) => !selected.includes(m) && !unsupportedReason(m, platform.os)).map((m) => m.name);
    if (others.length) console.log(`  ${c.dim('•')} not in your saved selection: ${others.join(', ')} (add them to MODULES in answers.env)`);
  }
  ...
  for (const m of modules) {
    for (const q of questionsOf(m, ctx)) (q.type === 'secret' ? secretKeys : known).add(q.key);
  }
  const saveTo = [...new Set([opts['save-answers'], usingSaved && !dryRun && SAVED_ANSWERS].filter(Boolean).map((f) => path.resolve(f)))];
  for (const key of answers.keys()) {
    if (known.has(key)) continue;
    const why = secretKeys.has(key) ? 'secrets come from environment variables only' : 'no question uses it any more';
    console.log(`  ${c.yellow('!')} answers file: ignoring ${key} (${why})${saveTo.length ? '; it is dropped when answers are saved' : ''}`);
  }
  ...
  for (const m of selected) {
    ...
    for (const q of qs) {
      ...
      const isNew = loaded.size > 0 && !answers.has(q.key) && q.type !== 'secret';
      const v = await ctx.ask(q);
      if (isNew && !interactive) console.log(`  ${c.dim('•')} new question ${q.key}: using the default "${v}"`);
    }
  }
  // --modules on the command line is a one-off: it replaces a saved selection only when
  // --save-answers asks for this run to be recorded.
  const keep = (k) => known.has(k) && !(k === 'MODULES' && opts.modules && !opts['save-answers']);
  for (const file of saveTo) saveAnswers(file, loaded, ctx.values, keep);
  if (!saveTo.length && dryRun && usingSaved) console.log(`\n${c.dim('A real run saves your answers to answers.env.')}`);
